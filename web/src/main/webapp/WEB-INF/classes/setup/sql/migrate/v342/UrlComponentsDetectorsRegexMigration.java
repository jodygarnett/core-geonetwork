...
public class UrlComponentsDetectorsRegexMigration extends JsonDatabaseMigration
    ...
    protected Map<String, String> setUpNewSettingValues() {
        ...
        fieldsToUpdate.put("/langDetector/regexp",
            "\"^(?:/.+)?/.+/([a-z]{2,3})/.+\"");
        fieldsToUpdate.put("/nodeDetector/regexp",
            "\"^(?:/.+)?/(.+)/[a-z]{2,3}/.+\"");
        fieldsToUpdate.put("/serviceDetector",
            "{\"regexp\":\"^(?:/.+)?/.+/[a-z]{2,3}/(.+)\",\"default\":\"catalog.search\"}");
        fieldsToUpdate.put("/baseURLDetector",
            "{\"regexp\":\"^((?:/.+)?)+/.+/[a-z]{2,3}/.+\",\"default\":\"/geonetwork\"}");
