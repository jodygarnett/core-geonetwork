...
(function() {
  ...
  module.controller('gnsCustom', [
    ...
    function($scope, $location, suggestService, $http, $translate,
      ...
      $scope.states.activeItem = $scope.viewTemplates[2].tooltip;
      ...
      $scope.toggleSideBar = function (e) {
        ...
        if($scope.toggleMenu){
          $('#filter1').removeClass('filter-toggle');
          $('#filter1').css("margin-left", '6rem');
        }else{
          $('#filter1').addClass('filter-toggle');
          $('#adv-1').removeClass().addClass('collapse');
        }
      ...
      $scope.toggleImage = function (e) {
        console.log('toggleImage....' + event.target.id);
        $('img.imgClass').each(function(e) {
          var id = '#img' + e;
          a = parseInt($(id).css("z-index"));
          console.log(id + ' z-index: '+ a)
          $(id).css("z-index", a + 1)
        });
      
        
        a = $('#'+e.target.id).css("z-index") - 1;
        $('#'+e.target.id).css("z-index", a);

        console.log('img0 z-index: '+$('#img0').css("z-index"))
        console.log('img1 z-index: '+$('#img1').css("z-index"))
      };

      $scope.toggleFilter = function (e) {
        /*$('a.filter').each(function(e) {
          var id = '#filter' + e;
          a = parseInt($(id).css("z-index"));
          $(id).css("z-index", a + 1);
        });
        ...
        $('#'+e.target.id).css("z-index", a);
        */
        console.log('$scope.toggleMenu: ' + $scope.toggleMenu);
        
        if($scope.toggleMenu){
          $('#filter0').css("z-index", 0);
          $('#filter1').css("z-index", 1);
        }else{
          $('#filter0').css("z-index", 1);
          $('#filter1').css("z-index", 0);
        }

        console.log('filter0 z-index: '+$('#filter0').css("z-index"));
        console.log('filter1 z-index: '+$('#filter1').css("z-index"));
