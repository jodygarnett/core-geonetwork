...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gmd="http://www.isotc211.org/2005/gmd"
  ...
  <!--<xsl:message>&#45;&#45;&#45;&#45;&#45;&#45;</xsl:message>
    ...
    <xsl:message>element config: <xsl:copy-of select="$config" /></xsl:message>
    <xsl:message>element type: <xsl:value-of  select="if ($config and $config/@use != '')
                              then $config/@use
                              else gn-fn-metadata:getFieldType($editorConfig, name(),
        name($theElement))"/></xsl:message>
    <xsl:message>element parent: <xsl:value-of  select="if ($config and $config/@parent != '')
                              then $config/@parent
                              else ''"/></xsl:message>
    <xsl:message>&#45;&#45;&#45;&#45;&#45;&#45;</xsl:message>-->

    <xsl:if test="$elementName = 'gmd:name'">
      <xsl:message>elementName: <xsl:value-of select="$elementName" /></xsl:message>
      <xsl:message>element getFieldType: <xsl:value-of select="gn-fn-metadata:getFieldType($editorConfig, name(),
        name($theElement), $xpath)" /></xsl:message>
    ...
    <xsl:call-template name="render-element">
      ...
      <!--<xsl:with-param name="type"
                      select="if ($config and $config/@use != '')
                              then $config/@use
                              else gn-fn-metadata:getFieldType2($editorConfig, name(),
