...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron">
   ...
   <sch:ns prefix="xsi" uri="http://www.w3.org/2001/XMLSchema"/>
	...
	<sch:pattern id="rule.ga.mdb.metadataparentidentifierpresent">
		...
		<sch:rule context="//mdb:metadataScope/mdb:MD_MetadataScope/mdb:resourceScope/mcc:MD_ScopeCode[contains('|feature|featureType|attribute|attributeType|',concat('|',@codeListValue,'|'))]">
			...
			<sch:assert test="$parentId" diagnostics="rule.ga.mdb.metadataparentidentifierpresent-failure-en"/>
			<sch:report test="$parentId" diagnostics="rule.ga.mdb.metadataparentidentifierpresent-success-en"/>
		</sch:rule>
	</sch:pattern>
	...
	<sch:pattern id="rule.ga.mdb.dataseturipresent">
		...
		<sch:rule context="/mdb:MD_Metadata/mdb:metadataScope/mdb:MD_MetadataScope/mdb:resourceScope/mcc:MD_ScopeCode[contains('|dataset||',concat('|',@codeListValue,'|'))]">
			<sch:let name="scopeCode" value="@codeListValue"/>
			...
			<sch:assert test="$dataseturi" diagnostics="rule.ga.mdb.dataseturipresent-failure-en"/>
			<sch:report test="$dataseturi" diagnostics="rule.ga.mdb.dataseturipresent-success-en"/>
		</sch:rule>
	</sch:pattern>
	...
	<sch:pattern id="rule.ga.mdb.metadataprofilepresent">
		...
		<sch:rule context="//mdb:MD_Metadata">
			<sch:let name="title" value="mdb:metadataProfile/cit:CI_Citation/cit:title/gco:CharacterString"/>
			<sch:let name="hasTitle" value="normalize-space($title) = 'Geoscience Australia Community Metadata Profile of ISO 19115-1:2014'"/>
			<sch:let name="edition" value="mdb:metadataProfile/cit:CI_Citation/cit:edition/gco:CharacterString"/>
   ...
   <sch:pattern id="rule.ga.mdq.dataqualityinfopresent">
	...
	<sch:pattern id="rule.ga.mrl.resourcelineagepresent">
		...
		<sch:rule context="//mdb:MD_Metadata">
			<sch:assert test="mdb:resourceLineage/mrl:LI_Lineage" diagnostics="rule.ga.mrl.resourcelineagepresent-failure-en">fail</sch:assert>
			<sch:report test="mdb:resourceLineage/mrl:LI_Lineage" diagnostics="rule.ga.mrl.resourcelineagepresent-success-en">pass</sch:report>
		</sch:rule>
		<sch:rule context="//mdb:resourceLineage/mrl:LI_Lineage">
			<sch:assert test="normalize-space(mrl:statement)" diagnostics="rule.ga.mrl.resourcelineagestatementpresent-failure-en">fail</sch:assert>
			<sch:report test="normalize-space(mrl:statement)" diagnostics="rule.ga.mrl.resourcelineagestatementpresent-success-en">pass</sch:report>
		</sch:rule>
	</sch:pattern>
	...
	<sch:pattern id="rule.ga.mco.metadatasecurityconstraints">
	...
	<!-- This rule doesn't make sense - will only execute if mri:MD_DataIdentification element is present. If it does execute it
	   tests if mri:MD_DataIdentification exists - will never fail. -->
	...
	<!-- What's the purpose of [parent::mdb:identificationInfo[parent::mdb:MD_Metadata]]?  The mri:MD_DataIdentification element always has
	parents elements of mdb:MD_Metadata/mdb:identificationInfo -->
	...
	<sch:pattern id="rule.ga.mco.legalconstraints">
		...
		<sch:rule context="//mco:MD_LegalConstraints/mco:accessConstraints">
			<!-- Is this test needed? XSD validation will fail if either @codeList or @codeListValue are missing -->
	...
	<sch:pattern id="rule.ga.mco.securityconstraints">
		...
		<sch:rule context="//mco:MD_SecurityConstraints/mco:classification">
		<!-- Is this test needed? XSD validation will fail if either @codeList or @codeListValue are missing -->
			<sch:assert test="normalize-space(mco:MD_ClassificationCode/@codeList) and normalize-space(mco:MD_ClassificationCode/@codeListValue)" diagnostics="rule.ga.mco.securityconstraintspresent-failure-en">fail</sch:assert>
			<sch:report test="normalize-space(mco:MD_ClassificationCode/@codeList) and normalize-space(mco:MD_ClassificationCode/@codeListValue)" diagnostics="rule.ga.mco.securityconstraintspresent-success-en">pass</sch:report>
		</sch:rule>
	</sch:pattern>
	...
	<sch:pattern id="rule.ga.mrd.resourceformat">
		...
		<!-- Note: mrd:MD_format elements with parent of mrd:distributionFormat are in a seperate test below -->
